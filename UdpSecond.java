...
//受信スレッドを作成・実行し,送信を担当
public class UdpSecond {
	final byte TTL = 1 ;//同一セグメント内部のみ到達可能とする ローカル
	final String MULTICASTADDRESS = ("224.0.0.3") ;
	...
	ListenPacketSecond lisnersecond;
	public void startListener()
	{
		try{
			lisnersecond = new ListenPacketSecond(soc);
			Thread lisner_thread = new Thread(lisnersecond);
	...
	// sendMsgsメソッド
	// マルチキャストパケットの送信を担当
	public void sendData(int dport,double x, double y, double battery, int port)
	{
		try{
...
//ListenPacketクラス
//マルチキャストパケットを受信
class ListenPacketSecond implements Runnable {
	MulticastSocket s = null;//初期化
	// コンストラクタマルチキャストスレッド受け取り
	public ListenPacketSecond(MulticastSocket soc){
		s = soc;
	}
