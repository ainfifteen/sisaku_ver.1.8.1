...
public class UdpThird {
	final byte TTL = 1 ;//同一セグメント内部のみ到達可能とする ローカル
	final String MULTICASTADDRESS = ("224.0.0.4") ;
	...
	ListenPacketThird lisnerthird;
	public void startListener()
	{
		try{
			lisnerthird = new ListenPacketThird(soc);
			Thread lisner_thread = new Thread(lisnerthird);
...
//ListenPacketクラス
//マルチキャストパケットを受信
class ListenPacketThird implements Runnable {
	MulticastSocket s = null;//初期化
	// コンストラクタマルチキャストスレッド受け取り
	public ListenPacketThird(MulticastSocket soc){
		s = soc;
	}
